...
const AuthorizedDetails = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isEditing
      ? `https://screeningstar-new.onrender.com/authorized-detail/update`
      : `https://screeningstar-new.onrender.com/authorized-detail/create`;
  ...
  const handleDelete = async (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`https://screeningstar-new.onrender.com/authorized-detail/delete?id=${id}&admin_id=${admin_id}&_token=${storedToken}`, requestOptions)
  ...
  return (
    ...
    <h2 className="text-2xl font-bold py-3 text-left text-[#4d606b] px-3 border">AUTHORIZED DETAILS</h2>
      ...
      <div className="flex space-x-4">
        ...
        <div className="w-2/5">
          <form className="space-y-4 ps-0 py-[30px] px-[51px] bg-white rounded-md" id="client-spoc" onSubmit={handleSubmit}>
            <div>
              <label htmlFor="name" className="block mb-2">Name of the Authorized Person</label>
              <input
                type="text"
                id="name"
                name="name"
          ...
          <div className="overflow-auto">
            <table className="min-w-full border-collapse border border-black rounded-lg">
              <thead className="rounded-lg">
                <tr className="bg-[#c1dff2] text-[#4d606b] rounded-lg whitespace-nowrap">
                  ...
                  <th className="py-2 px-4 border">Name of the Authorized Person</th>
            ...
            <div className="flex justify-center mt-4">
              {Array.from({ length: totalPages }, (_, i) => (
                <button key={i + 1} onClick={() => handlePageChange(i + 1)} className={`px-3 py-1 border rounded ${currentPage === i + 1 ? "bg-blue-500 text-white" : ""}`}>
                  {i + 1}
                </button>
              ))}
            </div>
